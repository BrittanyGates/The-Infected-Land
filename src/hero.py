...
from console import attack_or_block_successful, attack_or_block_unsuccessful, console, notification, \
    hero_or_villain_defeated
...
class Hero:
    def __init__(self, name: str, class_type: str, race_type: str, health: int = HEALTH, weapon_type: str = WEAPON_TYPE,
                 weapon_damage: int = BASE_SWORD_DAMAGE, armor_type: str = ARMOR_TYPE,
                 armor_defense: int = ARMOR_DEFENSE):
    ...
    def attack_villain(self, villain) -> None:
        ...
        while True:
            ...
            if user_input == "A":
                if random_num == 0:
                    console.print()
                    console.print(f"!! {hero.name} slashes at the {villain.name} with their sword !!",
                                  style=attack_or_block_successful, justify="center")
                    attack_sleep_print()
                    console.print()
                    console.print(f"-- {villain.name} fails to dodge the attack --", style=attack_or_block_unsuccessful,
                                  justify="center")
                    attack_sleep_print()
                    console.print()
                    console.print(
                        f"!! {self.weapon_type} does {self.weapon_damage - villain.armor_defense} points of damage !!",
                        justify="center")
                    villain.health -= (self.weapon_damage - villain.armor_defense)
                    console.print()
                    if villain.health <= 0:
                        attack_sleep_print()
                        console.print(f"!! {hero.name} defeats the {villain.name} !!", style=hero_or_villain_defeated,
                                      justify="center")
                        attack_sleep_print()
                        console.print()
                        console.print(f"== After the battle {hero.name} has {hero.health} health points ==",
                                      justify="center")
                        attack_sleep_print()
                        clear_screen()
                    else:
                    ...
                    console.print()
                    console.print(f"!! {hero.name} swings their sword at the {villain.name}, but misses !!",
                ...
                if random_num == 0:
                    ...
                    console.print()
                    attack_sleep_print()
                    console.print(f"-- The block stuns {villain.name} allowing {hero.name} to attack again --",
                                  justify="center")
                    attack_sleep_print()
                    clear_screen()
                else:
                    ...
                    console.print()
                    attack_sleep_print()
                    console.print(f"!! The block is unsuccessful !!", style=attack_or_block_unsuccessful,
                                  justify="center")
                    console.print()
                    attack_sleep_print()
                    clear_screen()
